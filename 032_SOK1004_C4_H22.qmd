...
```{r}
#Forkorter navnene på verdiene
df[df =="10-19 sysselsatte"] <- "10-19"
df[df =="20-49 sysselsatte"] <- "20-49"
df[df =="10-49 sysselsatte"] <- "10-49"
df[df =="50-99 sysselsatte"] <- "50-99"
df[df =="100-199 sysselsatte"] <- "100-199"
df[df =="200-499 sysselsatte"] <- "200-499"
df[df =="500 sysselsatte og over"] <- "500+"
# Rangerer verdiene i stigende rekkefølge
df$sysselsatte <- factor(df$sysselsatte,  levels = c("10-19", 
                                                     "10-49", 
                                                     "20-49", 
                                                     "50-99", 
                                                     "100-199", 
...
df %>%
  ...
  #Lager plot med sysselsatte og kostnad
  ggplot(aes(x = sysselsatte, 
             y = value, 
             fill = industri)) +
  
  #Viser kostnad som en andel av 1
  geom_bar(position = "fill", 
           stat = "identity") +
  
  #Legger til tittel, og endrer farger
  ggtitle("Fordeling av totale FoU-kostnader",
          subtitle = "etter antall ansatte og type næring") +
  scale_fill_viridis_d() +
  labs(fill = "Næring")+
  labs(y = "Andel av totale kostnader", 
       x="Ansatte") +
...
df %>%
  ...
  #Viser fordelingen av kostnad som en andel av 1
  geom_bar(position = "fill", 
           stat = "identity") +
  facet_wrap(~industri) +
  
  #Legger til tiittel, legger legend nederst og endrer farger
  ggtitle("Fordeling av FoU-kostnader i ulike næringer",
          subtitle = "etter antall ansatte og kostnadstsype") +
  scale_fill_viridis_d() +
  labs(fill = "", 
       y = "Kostnad", 
       x="Ansatte") +
...
#Måtte endre størrelse på tekst i legend og vinkling på x-akse for at det skulle være plass
